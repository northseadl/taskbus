...
func TestRabbitMQ_Concurrency(t *testing.T) {
	...
	topic := "it.concurrent"
	var processed int64
	stop, err := c.MQ().Consume(ctx, topic, "g1", func(ctx context.Context, m tq.Message) error {
	...
	n := 100
	for i := 0; i < n; i++ {
		_ = c.MQ().Publish(ctx, tq.Message{Topic: topic, Key: time.Now().Format(time.RFC3339Nano), Body: []byte("x")})
	}
