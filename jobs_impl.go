...
func (j *jobs) Register(job Job) { if job != nil { j.reg.Store(job.Name(), job) } }

func (j *jobs) Enqueue(ctx context.Context, jobName string, payload []byte, opts ...EnqueueOption) error {
	if jobName == "" { return fmt.Errorf("job name empty") }
	o := &enqueueOpts{}
	...
	if o.delay > 0 { return j.c.mq.PublishDelay(ctx, msg, o.delay) }
...
func (j *jobs) StartWorkers(ctx context.Context, groups map[string]int, mws ...JobMiddleware) (func(context.Context) error, error) {
	if len(groups) == 0 { groups = map[string]int{"default": 1} }
...
func (j *jobs) handle(ctx context.Context, msg Message) error {
	...
	name := msg.Topic
	if len(name) > 4 && name[:4] == "job." { name = name[4:] }
	v, ok := j.reg.Load(name)
	if !ok { return fmt.Errorf("job not registered: %s", name) }
	job := v.(Job)
...
func (j *jobs) retryMiddleware() Middleware {
	...
	return func(next Handler) Handler {
		return func(ctx context.Context, m Message) error {
			...
			attempt := 0
			if m.Headers != nil { if s, ok := m.Headers["x-attempt"]; ok { if n, e := strconv.Atoi(s); e == nil { attempt = n } } }
...
func copyHeaders(h map[string]string) map[string]string {
	if len(h) == 0 { return map[string]string{} }
